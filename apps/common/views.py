...
from .forms import EmailSettingForm, LDAPSettingForm, BasicSettingForm, \
    TerminalSettingForm, VerifySettingForm
...
class VerifySettingView(AdminUserRequiredMixin, TemplateView):
    form_class = VerifySettingForm
    template_name = "common/verify_setting.html"

    def get_context_data(self, **kwargs):
        context = {
            'app': _('Settings'),
            'action': _('Verify setting'),
            'form': self.form_class(),
        }
        kwargs.update(context)
        return super().get_context_data(**kwargs)
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            msg = _("Update setting successfully, please restart program")
            messages.success(request, msg)
            return redirect('settings:verify-setting')
        else:
            context = self.get_context_data()
            context.update({"form": form})
            return render(request, self.template_name, context)
